# Feature Cloud App MI4People_C4R_EDAqueries_Five1

Diese FeatureCloud-App dient als Analysewerkzeug zur explorativen Untersuchung einer biomedizinischen Wissensdatenbank, die in Neo4j gespeichert ist. Die App verbindet sich direkt mit der Datenbank, führt eine Reihe vordefinierter Cypher-Abfragen aus, um statistische Kennzahlen und strukturelle Informationen über den Graphen zu extrahieren, und speichert die Ergebnisse in einer einzigen HDF5-Datei (queries_output.h5). Die Hauptintention dieser App ist es, einen schnellen und reproduzierbaren Überblick über die Zusammensetzung, die Verbindungen und potenzielle "Hotspots" (z.B. stark vernetzte Knoten) des Graphen zu erhalten, ohne manuelle Abfragen durchführen zu müssen. 

## Überblick der Cypher-Abfragen
- `nodes_per_label`: Zählt alle Knoten pro Label (z.B. wie viele Protein-Knoten existieren).
- `nodes_per_patient`: Zählt, wie viele Knoten jedes Typs direkt mit Patienten (Biological_sample) verknüpft sind.
- `source_target`: Listet alle Beziehungstypen zwischen den verschiedenen Knoten-Labels auf und zeigt so das grundlegende Graphen-Schema.
- `patients_overlap`: Findet gemeinsame Merkmale (Gene, Phänotypen etc.), die von zwei oder mehr Patienten geteilt werden.
- `patient_subgraph_overlap`: Berechnet für jedes Patientenpaar die Anzahl der gemeinsamen Gene, Proteine und Phänotypen, um die Ähnlichkeit zwischen ihnen zu quantifizieren.
- `diseases`: Identifiziert Proteine und Gene, die exklusiv mit nur einer einzigen Krankheit verknüpft sind (potenzielle spezifische Biomarker).
- `proteins`: Erstellt für jedes Protein ein Profil, indem die Anzahl seiner Verbindungen zu ontologischen Klassen (z.B. Pathways, Diseases) gezählt wird.
- `links_per_node`: Identifiziert die am stärksten vernetzten Knoten ("Hubs") im Graphen, indem die Anzahl ihrer ausgehenden Beziehungen gezählt wird.
- `patients_diseases`: Zählt die Anzahl der verschiedenen Krankheiten, die mit jedem einzelnen Patienten verknüpft sind.
