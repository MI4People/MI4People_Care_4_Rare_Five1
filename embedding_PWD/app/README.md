# FeatureCloud App: MI4People_C4R_Embeddings_PWD

Diese App ist eine Weiterentwicklung der **MI4People_C4R_Embeddings**-App- Ihr Hauptzweck ist es, die App auf einer spezifischen Patientenkohorte zu testen. Die Pipeline der App wurde daher um einen initialen Selektionensschritt erweitert. Anstatt alle `Biological_samples`-Knoten zu verwenden, identifiziert die App zunächste eine Patientenkohorte, die mit einer spezifischen Krankheits-ID verknüpft ist. 

Ein weiterer Unterschied ist, dass diese App die **Client-Side-Aggragation**-Strategie nutzt, um Timeout- und Speicherfehler zu vermeiden. Die rechen- und speicherintensive Arbeit der Aggregation (das Zusammenfassen von Daten zu Listen) werden im Python-Skript (Client) zusammenfegührt. 
